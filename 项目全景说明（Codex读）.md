...
- 基础标签是与四维规则相同的规则风险分。
- <code>real_labels_1500.json</code> 有 1500 行，但训练只使用其中 127 条带 <code>source_url</code> 的公开事件覆盖对应月份的规则标签。
- 其余 1373 个月份是“未确认”，不是“真实无灾”。
...
| 文件 | 行数 | 当前含义 |
|---|---:|---|
| <code>risk_event_labels.json</code> | 198 | 173 条带来源 URL 的公开事件材料 + 25 条样例；当前模型训练不直接读取此表 |
| <code>real_labels_1500.json</code> | 1500 | 107 条带来源 URL 的公开事件 + 1393 条未确认月份；模型直接读取 |
...
- 未核验资料不能产生综合授信风险分。
- 尽调接口不输出个人敏感字段。
- 标签说明必须承认 127 条来源事件和 1373 个未知月份。
