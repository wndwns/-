/**
 * Google Earth Engine - MODIS NDVI 25县月度导出模板
 * ============================================================================
 *
 * 使用方法:
 ...
 *   5. 放到: public_data/raw/modis/ndvi/ndvi_modis_real_2020_2024.csv
 *   6. 运行转换:
 ...
 *
 * 数据源: MODIS/061/MOD13Q1 (250m 16-day NDVI)
 * 输出: 25 县 × 5 年 × 12 月 = 1500 行
 * 字段: region_id, region_name, date, ndvi
 */

// ---- 25 个高原牧区示范县 (region_id 与 public_data/region_list.csv 一致) ----
var sites = [
...
// Turn lon/lat attributes into point geometries. reduceRegions needs real
// geometries; keeping lon/lat only as properties returns empty NDVI values.
...
var monthlyRows = ee.FeatureCollection(monthOffsets.map(function(offset) {
  ...
  return reduced.map(function(f) {
    return f.set({
      ...
      'ndvi': f.get('mean')
    }).select(['region_id', 'region_name', 'date', 'ndvi']);
  });
})).flatten();
...
Export.table.toDrive({
  ...
  description: 'ndvi_modis_real_2020_2024',
  fileFormat: 'CSV',
  selectors: ['region_id', 'region_name', 'date', 'ndvi']
});
