...
# 参数量级依据（2026-08-16 标注）：
# - 需求系数 1.10 / 可达 0.70 / 价格 1.15（标准雪灾）：对应青南牧区典型雪灾下
#   牲畜维持需求上升约一成、草场有效采食面积降至约七成、饲草到场价上升约一成五；
#   量级与项目历史事件集（2014-2019 减灾网/年鉴 26 县雪灾记录）中牲畜损失与饲草
#   短缺占比相符，详见《授信情景参数依据与敏感性分析.md》。
# - 复合极端 1.20 / 0.50 / 1.30：压力月扩展到 1、2、12 月，需求/可达/价格同时恶化，
#   仅作压力提示、不生成第二金额。
# 参数为演示口径的显式标定值，可在 case 级覆盖或按权威文献调整（敏感性区间见文档）。
