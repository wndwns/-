...
- 来源支持事件 128 条（附公开 URL 或经核验的年鉴页码凭证），其余 1372 个月份为“未确认”，不能当作无事件
- 模型类型 `ml_hybrid`（引擎 xgboost，RF+Ridge 兜底）；评估指标（MAE/RMSE/R² 等）是“对规则弱标签的拟合度”，不代表真实灾害、损失或逾期预测能力
- 事件月响应检验（2026-08-16）：128 条真实事件月的模型预测分县内百分位均值 0.811、93.8% 进入本县前 50%、事件月平均高出同县其他月份约 12 分，严重度单调（高>中>低）；时间前向（2023+ 未见月份）百分位 0.636、方向一致。
...
`risk_event_labels` 现有 219 条（194 条来源支持事件材料 + 25 条样例）；模型训练读取 `real_labels_1500.json`，其中 128 条附公开 URL 或经核验年鉴页码凭证的事件作为来源支持样本，其余 1372 个月份为未确认状态。`is_real_label=true` 不等于真实业务标签；不伪造标签。
